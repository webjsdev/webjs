...
The popular libraries built on top of web components all picked a different default. lit's [`LitElement` attaches a shadow root](https://lit.dev/docs/components/shadow-dom/) in its constructor unless you override `createRenderRoot()` to return `this`. [Stencil's components default to shadow DOM](https://stenciljs.com/docs/styling) (the `stencil generate` CLI emits shadow-enabled components, and the `@Component` decorator's `shadow` field defaults to true). [Microsoft's FAST automatically attaches a `ShadowRoot`](https://fast.design/docs/1.x/fast-element/working-with-shadow-dom/) and renders the template into it. Because these three libraries are what most developers (and most AI training data) treat as "how you write web components," the perception has shifted: people assume shadow DOM is the web-components default. It is not. It is a library convention that the platform itself does not share.

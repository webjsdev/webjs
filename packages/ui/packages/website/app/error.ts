 ...
 * Only @webjskit/core; no _lib helpers, no custom elements, no shared
 * components.
 *
...
export default function ErrorBoundary({ error }: { error: unknown }) {
  ...
  return html`
    <section class="min-h-[60vh] flex flex-col items-start justify-center max-w-2xl mx-auto px-6 py-20">
      <div class="text-xs font-mono uppercase tracking-widest text-brand mb-4">500 &middot; server error</div>
      ...
      <pre class="w-full mb-8 text-sm overflow-x-auto"><code>${message}</code></pre>
      <a href="/" class="inline-flex items-center gap-1.5 text-brand no-underline font-medium hover:underline">&larr; Back home</a>
    </section>
  `;
}

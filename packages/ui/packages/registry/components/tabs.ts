...
const TABS_TRIGGER_CLASS = [
  ...
  // The underline variant must override BOTH the active border AND bg
  // in light + dark — the active tab is marked by the after:opacity-100
  // underline (see the fourth row), not by a surrounding box. Previously
  // border-transparent was only set under `dark:`, so light-mode users
  // saw the active tab framed by a border-input outline that looked
  // exactly like the default variant — the underline variant rendered
  // as "outline" by mistake.
  'group-data-[variant=underline]/tabs-list:bg-transparent group-data-[variant=underline]/tabs-list:data-[state=active]:bg-transparent group-data-[variant=underline]/tabs-list:data-[state=active]:border-transparent dark:group-data-[variant=underline]/tabs-list:data-[state=active]:border-transparent dark:group-data-[variant=underline]/tabs-list:data-[state=active]:bg-transparent',

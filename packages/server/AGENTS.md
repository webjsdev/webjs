...
| File | What it owns |
|---|---|
| `dev.js` | The request handler. File serving, TypeScript stripping (Node 24+ built-in `module.stripTypeScriptTypes`, backed by the `amaro` package; non-erasable syntax is rejected at request time with a 500, no fallback), **server-file guardrail**, live reload via SSE |

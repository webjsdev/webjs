...
| `html-cache.js` | Server HTML response cache (ISR for no-build, #241). OPT-IN per page via `export const revalidate = N` (seconds), read by `readRevalidate`. `readHtmlCache` / `writeHtmlCache` use the shared store under a URL-keyed namespace (`htmlCacheKey`, query-order-normalized, generation-folded so `revalidateAll` flushes in one bump). `isCacheableResponse` is the defense-in-depth guard (status 200, not streamed, CSP off, no non-framework `Set-Cookie`; the framework `webjs_csrf` cookie alone does not block). The cache LOOKUP + opt-in read live in `ssrPage`; the WRITE is a response-funnel step (`commitHtmlCache`) so it re-checks the guards against the FINAL post-middleware response. `revalidatePath(path)` / `revalidateAll()` are the server-side on-demand eviction surface (distinct from core's client-side `revalidate()`) |

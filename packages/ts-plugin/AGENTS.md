...
```
src/
  index.js         The whole plugin. Single-file by design — tsserver
...
1. **Tag recognition is gated on the import graph of the current file.**
   ...
   available — runtime would fail too, and surfacing the diagnostic is
   the correct prompt to add the import.
2. **Static (non-interpolated) attribute text is not type-checked.**
   At runtime it's plain template text — any string is valid. Only

...
export async function writeSaasFiles(appDir, opts = {}) {
  ...
  await writeFile(join(appDir, 'lib', 'auth.server.ts'), [
    ...
    "// safe for production, so we fail fast if it is missing there.",
    "const authSecret = process.env.AUTH_SECRET ?? 'dev-insecure-secret-change-me';",
  ...
  await writeFile(join(appDir, 'app', 'dashboard', 'page.ts'), [
    ...
    "    <div class=${cardClass()}>",
    "      <div class=${cardHeaderClass()}>",
    "        <h1 class=${cardTitleClass()}>Welcome, ${user?.name || user?.email}!</h1>",
  ...
  await writeFile(join(appDir, 'app', 'dashboard', 'settings', 'page.ts'), [
    ...
    "    <div class=${cardClass()}>",
    "      <div class=${cardHeaderClass()}>",
    "        <h1 class=${cardTitleClass()}>Account</h1>",

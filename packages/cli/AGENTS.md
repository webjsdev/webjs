...
3. **Drizzle + SQLite is wired up for ALL templates.** The `db/` folder
   ...
   `webjs start`, #550/#725). `webjs create` generates + applies the initial
   migration, so the database is real on the first boot.

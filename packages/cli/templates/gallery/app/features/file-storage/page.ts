...
export default function FileStorageExample({
  ...
  return html`
    ...
    <form method="post" enctype="multipart/form-data" class="flex flex-wrap gap-3 items-center mb-4">
      ...
      <button type="submit"
        class="px-4 py-2 rounded-xl bg-accent text-accent-foreground font-semibold text-sm border-0 cursor-pointer transition-all hover:bg-accent/90 active:scale-[0.97]">Upload</button>
    </form>
    ${actionData?.error
    ...
    ${key
      ? html`
        <div class="px-4 py-3 rounded-xl bg-card border border-border text-sm">
          ...
          <a class="text-accent no-underline ml-2" href="/features/file-storage/file/${key}">download</a>
        </div>`
      : ''}
  `;
}

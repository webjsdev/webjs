...
const field = (label: string, name: string, input: unknown, error?: string) => html`
  <div class="grid gap-1.5">
    <label for=${name} class="text-[13px] font-medium text-fg-muted">${label}</label>
    ${input}
    ${error ? html`<p class="m-0 text-[12.5px] text-destructive">${error}</p>` : ''}
  </div>
`;

const inputCls = 'w-full bg-bg border border-border rounded-xl px-3 py-2 text-[15px] text-fg outline-none transition-colors focus:border-accent placeholder:text-fg-subtle';
...
export default function FormsFeature({ searchParams, actionData }: { searchParams: Record<string, string | undefined>; actionData?: Result }) {
  if (searchParams.sent) {
    return html`
      ...
      <div class="max-w-[460px] grid gap-3 p-6 rounded-2xl bg-bg-elev border border-border text-center">
        <span class="mx-auto grid place-items-center w-12 h-12 rounded-2xl bg-accent-tint text-accent">
        ...
        <p class="m-0 text-lg font-semibold text-fg">Message sent</p>
        <p class="m-0 text-sm text-fg-muted">Thanks, we got it. <a class="text-accent" href="/features/forms">Send another</a>.</p>
      </div>
    `;
  }
  ...
  return html`
    ...
    <p class="text-fg-muted mb-5 max-w-[460px]">A real <code>&lt;form&gt;</code> posting to this page's <code>action</code>. It works with JS off; validation errors come back on <code>actionData</code>.</p>
    <form method="post" action="" class="max-w-[460px] grid gap-4 p-5 rounded-2xl bg-bg-elev border border-border">
      ...
      <button type="submit" class="justify-self-start px-4 py-2 rounded-xl bg-accent text-accent-fg font-semibold text-sm border-0 cursor-pointer transition-all hover:bg-accent-hover active:scale-[0.97]">Send message</button>
    </form>
  `;
}

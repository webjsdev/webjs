...
export default function OptimisticUiFeature() {
  return html`
    ...
    <p class="text-muted-foreground mb-4">The imperative <code>optimistic(signal, value, action)</code> form: the UI flips instantly and rolls back if the action fails. For the declarative list form (add / remove with rollback) in a full app, see <a class="text-accent" href="/examples/todo">/examples/todo</a>.</p>

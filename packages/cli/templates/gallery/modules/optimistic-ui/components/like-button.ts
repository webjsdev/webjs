  ...
  render() {
    ...
    return html`
      <button @click=${() => this.toggle()} aria-pressed=${liked ? 'true' : 'false'}
        class="inline-flex items-center gap-2 px-4 py-2 rounded-full font-semibold text-sm border cursor-pointer transition-all active:scale-[0.97] ${liked ? 'bg-accent text-accent-fg border-accent' : 'bg-bg-elev text-fg border-border hover:border-border-strong'}">

...
- **DX:** TypeScript with zero build, `AGENTS.md` contract, `CLAUDE.md`, live reload in dev, optional esbuild bundle for prod, plus a two-plugin tsserver pair shipped in the scaffold's `tsconfig.json` — `ts-lit-plugin` for type-checked `` html`…` `` templates and `@webjskit/ts-plugin` on top for webjs-aware tag/attribute intelligence (go-to-definition, attribute auto-complete from `static properties`, silenced "Unknown tag" diagnostics, all gated by the file's import graph). Neither plugin is required for the framework to run — they're editor-only — but they go together: install both or neither, since `@webjskit/ts-plugin` wraps `ts-lit-plugin`.

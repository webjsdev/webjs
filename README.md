...
- **No build step you run.** `.ts` files served directly. Node 24+ is the minimum runtime, and the dev server strips types via Node's built-in `module.stripTypeScriptTypes` (position-preserving, no sourcemap, near-zero overhead). TypeScript must be erasable. Non-erasable constructs (enums, value-carrying namespaces, constructor parameter properties, legacy decorators with `emitDecoratorMetadata`) are rejected at request time with a 500 and a clear remediation message; the `erasable-typescript-only` and `no-non-erasable-typescript` lint rules catch them at commit time. Edit, refresh, done.

...
3. **Scaffold + skill sync (when a feature changes what apps should do).** The scaffold `webjs create` emits is a MINIMAL starting point (a home page, a root layout, db wiring) plus the one cross-agent skill at `packages/cli/templates/.agents/skills/webjs/` (SKILL.md + references), grown in place by the agent. There is no feature gallery. So when a WebJs feature is added or changed, ask: does the generator (`packages/cli/lib/{create,saas-template}.js`) or the agent skill (`.agents/skills/webjs/SKILL.md` + its `references/`) need to move so a freshly scaffolded app and the skill teach the new reality? Verify by generating an app and running `generate + boot + webjs check` (the generators emit strings, so an escaping bug only shows in a freshly generated app). See `agent-docs/framework-dev.md`.

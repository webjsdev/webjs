...
**Order vs `webjs.redirects` (loop avoidance).** The declarative redirects run FIRST, then the survivor is slash-canonicalized. So an explicit `webjs.redirects` rule always wins, and the two surfaces compose without a loop (a redirect `destination` is the app author's literal; keep it consistent with the slash policy). Applied at the very START of request handling (in `dev.js`'s `produce()`, right after `applyRedirects`, before routing / SSR), so the canonical URL reaches the router. Mechanism: `readTrailingSlashPolicy` / `applyTrailingSlash` in `packages/server/src/redirects.js`.

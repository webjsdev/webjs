...
10. **TypeScript must be erasable.** Set `compilerOptions.erasableSyntaxOnly: true`. No `enum`, no `namespace` with values, no constructor parameter properties, no legacy decorators with `emitDecoratorMetadata`, no `import = require`. The framework strips types via Node 24+'s built-in `module.stripTypeScriptTypes` (position-preserving, no sourcemap). Non-erasable syntax is rejected at request time with a 500 and a clear remediation message; there is no esbuild fallback. The `erasable-typescript-only` check verifies the tsconfig flag, and `no-non-erasable-typescript` does an AST-level scan for offending source. See `agent-docs/typescript.md` for erasable equivalents.

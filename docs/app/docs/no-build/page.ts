...
...</pre>
    ...
    <p>The browser can't resolve <code>import dayjs from 'dayjs'</code> on its own. webjs follows the Rails 7 + <code>importmap-rails</code> model: bare specifiers resolve via importmap to URLs the framework proxies from esm.sh (fallback jspm.io) and caches to <code>.webjs/vendor/</code> at the project root:</p>
    <ol>
      ...
      <li>On first request to <code>/__webjs/vendor/dayjs@1.11.13.js</code>, fetch from <code>https://esm.sh/dayjs@1.11.13?target=es2022</code>, write the response to <code>.webjs/vendor/dayjs@1.11.13.js</code>, and serve the bytes.</li>
    ...
    <p>The bundle itself is produced once by esm.sh (which runs an aggressive transformation pipeline server-side) and shipped to your <code>.webjs/vendor/</code> cache. webjs never runs a bundler on your machine; <code>@webjsdev/server</code> has zero esbuild dependency. The CDN handles the messy real-world cases (mixed CJS/ESM packages, <code>require()</code> in code that claims to be ESM, missing extensions, transitive non-ESM deps) so your local toolchain stays small.</p>
    <p>For air-gapped or compliance-heavy deploys, run <code>webjs vendor pin</code> to populate <code>.webjs/vendor/</code> ahead of time, commit it, and your production server never needs CDN access at runtime. Mirrors Rails' <code>bin/importmap pin --download</code> workflow.</p>

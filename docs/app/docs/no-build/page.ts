...
export default function NoBuild() {
  return html`
    ...
    <ol>
      ...
      <li>If the file is <code>.ts</code> / <code>.mts</code>, runs Node 24+'s built-in <code>module.stripTypeScriptTypes</code>. This is whitespace replacement: every <code>(line, column)</code> in the source maps to the same position in the output, so no sourcemap is needed and stack traces are byte-exact. Non-erasable syntax (<code>enum</code>, value-carrying <code>namespace</code>, parameter properties, legacy decorators with <code>emitDecoratorMetadata</code>) is rejected at request time with a 500 and a clear remediation message. The scaffolded <code>tsconfig.json</code> turns on <code>erasableSyntaxOnly: true</code> by default, which rejects all of these in your own code at edit time, and the <code>no-non-erasable-typescript</code> lint rule scans <code>.ts</code> source as a second line of defense. Published npm packages ship <code>.js</code> with sidecar <code>.d.ts</code> type files, so the stripper only ever runs on your own code in practice.</li>
...
...</pre>
    ...
    <p>The browser can't resolve <code>import dayjs from 'dayjs'</code> on its own. webjs follows the Rails 7 + <code>importmap-rails</code> model: bare specifiers resolve via importmap to URLs the framework proxies from esm.sh (fallback jspm.io) and caches to <code>vendor/javascript/</code> at the project root:</p>
    <ol>
      ...
      <li>For each discovered package, add an importmap entry: <code>{ "dayjs": "/__webjs/vendor/dayjs@1.11.13.js" }</code>. The version comes from <code>node_modules/dayjs/package.json</code>.</li>
      <li>On first request to <code>/__webjs/vendor/dayjs@1.11.13.js</code>, fetch from <code>https://esm.sh/dayjs@1.11.13?target=es2022</code>, write the response to <code>vendor/javascript/dayjs@1.11.13.js</code>, and serve the bytes.</li>
      <li>Subsequent requests are pure file reads with zero network involvement. The cache is committed to source control, so production deploys are offline-capable without any CDN dependency at runtime.</li>
    </ol>
    <p>esm.sh handles the CJS-to-ESM conversion server-side, inlines transitive deps, and serves minified output. Native modules and server-only packages (<code>node:*</code>, <code>@prisma/client</code>) fail at the CDN with a clear 404 and never get an importmap entry. That's the right behaviour: server packages should never reach the browser.</p>

    <h2>Why CDN-proxied vendor in a no-build framework?</h2>
    <p>This is an architectural decision worth calling out. A stricter "browser-native ESM only" interpretation of no-build would refuse to do anything for npm packages and push importmap management entirely onto the user. Rails 7+ with <code>importmap-rails</code> is the canonical example, and it requires you to run <code>bin/importmap pin &lt;pkg&gt;</code> for every dependency. webjs auto-discovers bare specifiers by scanning your app's source, so you skip the manual pin step. <code>import dayjs from 'dayjs'</code> works the moment you <code>npm install dayjs</code>, with no other action required.</p>
    <p>The bundle itself is produced once by esm.sh (which runs an aggressive transformation pipeline server-side) and shipped to your <code>vendor/javascript/</code> cache. webjs never runs a bundler on your machine; <code>@webjsdev/server</code> has zero esbuild dependency. The CDN handles the messy real-world cases (mixed CJS/ESM packages, <code>require()</code> in code that claims to be ESM, missing extensions, transitive non-ESM deps) so your local toolchain stays small.</p>
    <p>For air-gapped or compliance-heavy deploys, run <code>webjs vendor pin</code> to populate <code>vendor/javascript/</code> ahead of time, commit it, and your production server never needs CDN access at runtime. Mirrors Rails' <code>bin/importmap pin --download</code> workflow.</p>
    <p>The framework itself stays no-build in the sense that matters most. Source equals runtime for <code>@webjsdev/*</code> packages and for your own app code, no compile step before deploy, no output directory, no bundle hashes to invalidate. Vendor packages come pre-built from esm.sh; you cache the result alongside your source.</p>

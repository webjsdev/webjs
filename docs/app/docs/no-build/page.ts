...
...</pre>
    ...
    <p>Native modules and server-only packages (<code>node:*</code>, <code>better-sqlite3</code>) are filtered out by the scanner (they're imported only from <code>.server.{js,ts}</code> / <code>route.{js,ts}</code> / <code>middleware.{js,ts}</code> files, which the scanner skips). Server packages never reach the browser.</p>

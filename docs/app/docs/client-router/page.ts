...
export default function ClientRouter() {
  return html`
    ...
    <p><strong>Wire-byte optimization</strong>: the router sends an <code>X-Webjs-Have</code> request header listing the boundary segments it already has. The server walks the target page's layout chain innermost-to-outermost, short-circuits at the first match, and returns only the divergent fragment wrapped in that layout's boundary pair. Outer layouts are never re-serialized for same-shell navigations, and a reduced response carries <code>Vary: X-Webjs-Have</code> so a shared cache can never serve it to a full-page navigation.</p>

...
webjs create my-app --template saas</pre>
    ...
    <p><strong>The scaffold is a minimal starting point, grown in place</strong> (the Remix 3 model). Every scaffold is intentionally small: a home page, a neutral-palette root layout, and database wiring, with no feature gallery, no <code>app/features/</code>, no <code>app/examples/</code>, and no backend-features showcase. The framework context you need to build ships as one cross-agent skill, <code>.agents/skills/webjs/SKILL.md</code> (a routing skill), alongside <code>AGENTS.md</code> and the workflow rules in <code>.agents/rules/workflow.md</code>. Read the skill first, then build the app the user asked for directly on top of the scaffold, editing the home page, layout, and schema in place rather than deleting demo routes.</p>

...
webjs create my-app --runtime bun</pre>
    <p>A Bun app commits <code>bun.lock</code> instead of <code>package-lock.json</code>, sets <code>trustedDependencies</code> so the SQLite driver's native postinstall runs (Bun skips postinstalls by default), ships an <code>oven/bun</code> Dockerfile + CI, and its <code>dev</code> / <code>start</code> scripts force <code>bun --bun</code> so the server runs on Bun rather than the <code>webjs</code> bin's Node shebang. Run it with <code>bun --bun run dev</code>. One flag-forwarding difference to note: Bun forwards flags directly (<code>bun create webjs my-app --template api</code>), while npm needs the <code>--</code> separator (<code>npm create webjs@latest my-app -- --template api</code>). <code>--runtime node</code> (the default) is unchanged.</p>

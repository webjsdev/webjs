...
# → http://localhost:3000</pre>
    ...
    <h2>How It Works</h2>
    <ul>
      <li><strong>TypeScript:</strong> Node 24+ strips types natively via <code>module.stripTypeScriptTypes</code> (whitespace replacement, byte-exact line + column preservation, no sourcemap shipped). Every <code>.ts</code> file, whether server-side or browser-fetched, goes through the same stripper, so SSR and hydration produce identical JS. The transform is cached by mtime. Non-erasable syntax (<code>enum</code>, value-carrying <code>namespace</code>, parameter properties, legacy decorators with <code>emitDecoratorMetadata</code>) is rejected at request time with a 500 and a clear remediation message. The <code>erasable-typescript-only</code> and <code>no-non-erasable-typescript</code> lint rules catch these at commit time.</li>

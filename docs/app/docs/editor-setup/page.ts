...
export default function EditorSetup() {
  return html`
    ...
    <p><strong><code>@webjsdev/ts-plugin</code> is editor-only, not required for the framework to run.</strong> As of <code>@webjsdev/ts-plugin@0.4.0</code> it bundles <code>ts-lit-plugin</code> internally (loads it programmatically inside its <code>create(info)</code> factory), so users install one plugin and list one plugin in <code>tsconfig.json</code>. The scaffold wires this up automatically.</p>

    <p>This page covers three layers of intelligence. The first works out of the box. The last two arrive together once <code>@webjsdev/ts-plugin</code> is installed:</p>
    <ol>
  ...
  "compilerOptions": {
    ...
    <ul>
      ...
      <li><code>plugins</code>: one entry. <code>@webjsdev/ts-plugin@0.4.0+</code> bundles <code>ts-lit-plugin</code> internally and loads it programmatically, so no separate <code>ts-lit-plugin</code> entry is needed.</li>
    </ul>
...
StudentCard.register('student-card');</pre>
    ...
    <p><code>@webjsdev/ts-plugin</code> bundles <a href="https://www.npmjs.com/package/ts-lit-plugin" target="_blank">ts-lit-plugin</a> as a runtime dependency and loads it programmatically inside its factory, so once you install <code>@webjsdev/ts-plugin</code> you also get:</p>
    <ul>
...
//                                    ^^^ squiggle: \`number\` is not assignable to \`Student\`.</pre>
    ...
    <h2>Layer 3: <code>@webjsdev/ts-plugin</code> for webjs-aware intelligence</h2>
    ...
    <h3>Install</h3>
    <pre>npm i -D @webjsdev/ts-plugin</pre>
    <p>The baseline <code>tsconfig.json</code> at the top of this page already lists this single plugin. From <code>0.4.0</code> onward there's no separate <code>ts-lit-plugin</code> entry. It's bundled as a runtime dependency of <code>@webjsdev/ts-plugin</code> and loaded programmatically.</p>

    <p><strong>Upgrading from a pre-0.4.0 setup?</strong> Remove <code>ts-lit-plugin</code> from your <code>devDependencies</code> and from the <code>plugins</code> array in <code>tsconfig.json</code>, leaving only <code>{ "name": "@webjsdev/ts-plugin" }</code>. Run <code>npm install</code>, and <code>ts-lit-plugin</code> will reappear in <code>node_modules</code> as a transitive dep.</p>

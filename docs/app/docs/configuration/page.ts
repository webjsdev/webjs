...
class Checkout extends WebComponent {
  render() {
    ...
    <h3>Defense in depth: three layers</h3>
    <p>The naming-and-shim story by itself protects against <em>browser-runtime</em> reads of secrets. But a component's <code>render()</code> runs on the server during SSR with full access to <code>process.env</code> unless something stops it. Webjs ships three layers that catch this at increasing levels of certainty:</p>
    <ol>
      <li><strong>Lint rule (write time).</strong> <code>webjs check</code>'s <code>no-server-env-in-components</code> rule flags any <code>process.env.X</code> read in a component file when <code>X</code> is not <code>WEBJS_PUBLIC_*</code> and not <code>NODE_ENV</code>. Catches the static, dot-access case before the code is even committed.</li>
      <li><strong>SSR runtime proxy (server execution time).</strong> The framework installs a <code>Proxy</code> on <code>process.env</code> that is active only inside component lifecycle scopes (construction, attribute application, and <code>render()</code>, including any awaited work). Inside that scope, non-public keys read as <code>undefined</code>. Outside it (page functions, layouts, route handlers, middleware, server actions invoked via RPC), <code>process.env</code> behaves normally with full server access. Tracked via <code>AsyncLocalStorage</code>, so the filter follows async boundaries. This catches dynamic access (<code>process.env[varName]</code>), destructuring (<code>const { SECRET } = process.env</code>), and reads from helper modules transitively called from <code>render()</code>. The lint rule cannot statically detect those.</li>
      <li><strong>Browser shim (client runtime).</strong> The SSR-injected <code>window.process.env</code> object only contains <code>WEBJS_PUBLIC_*</code> values and <code>NODE_ENV</code>. Any other <code>process.env.X</code> read in the browser returns <code>undefined</code>, regardless of what the component code says.</li>
    </ol>
    <p>The three layers compose. If you bypass one (disable the lint rule, manually call an unwrapped render path, ship raw env to the browser), the others still catch the leak. None of them require you to mark a component as "client" or "server". Components are isomorphic by default; the framework knows when it's inside an SSR render and acts accordingly.</p>
    <p>The fix when a violation fires is always one of: rename to <code>WEBJS_PUBLIC_*</code> if the value is intended for the browser, or read it in a page function / server action / middleware and pass a derived value to the component as an attribute.</p>

...
npm run dev      # or: npm run start</pre>
    ...
    <p>Bun's auto-install fetches the <strong>latest</strong> version for a bare <code>import "zod"</code>, ignoring <code>package.json</code> and <code>bun.lock</code>. webjs closes that gap: an <code>onLoad</code> transform rewrites a declared dependency's bare specifier to an inline-versioned one (<code>zod</code> becomes <code>zod@&lt;pinned&gt;</code>, the exact <code>bun.lock</code> version when present, else the <code>package.json</code> range), which Bun's auto-install honors. So zero-install runs the versions your <code>package.json</code> declares, not whatever is latest. The rewrite is server-side only (it shapes what Bun fetches for SSR and server actions); the browser is still served bare specifiers resolved through the importmap. It is on by default. Opt out with <code>WEBJS_PIN=0</code> or <code>{ "webjs": { "pin": false } }</code>.</p>

...
export default function TypeScript() {
  return html`
    ...
    <p>The "no build" promise is literal: every position in source maps to itself in runtime. DevTools shows accurate stack traces without consulting a sourcemap. No bundler runs on your machine for app code; vendor packages resolve via importmap to esm.sh and cache to <code>vendor/javascript/</code> (Rails 7 + importmap-rails pattern).</p>

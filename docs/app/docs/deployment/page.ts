...
CMD ["npx", "webjs", "start"]</pre>
    ...
    <ul>
      ...
      <li>A <strong>pure Bun image</strong> is the simplest path: scaffold with <code>webjs create my-app --runtime bun</code> (or <code>bun create webjs my-app</code>) and the generated <code>Dockerfile</code> is already <code>FROM oven/bun:1</code> with <code>bun install</code> and <code>CMD ["bun", "--bun", "run", "start"]</code> (no Node in the image). This works because <code>webjs db</code> / <code>webjs test</code> resolve their tools (drizzle-kit, wtr) and spawn them with the current runtime instead of <code>npx</code> (#570; a pure Bun image has no <code>npx</code>), so the boot-time <code>webjs db migrate</code> runs under Bun with no Node toolchain. better-sqlite3 publishes a glibc prebuild that <code>bun install</code> fetches on that base, so no build toolchain is needed; the app's <code>trustedDependencies</code> lets its postinstall run.</li>
      <li>To <strong>serve on Bun</strong> (more requests/sec on the listening path) while keeping a Node base for the image build, drop the Bun binary into a Node image with <code>COPY --from=oven/bun:1-alpine /usr/local/bin/bun /usr/local/bin/bun</code> and start with <code>bun node_modules/@webjsdev/cli/bin/webjs.js start</code>. <code>startServer</code> auto-selects the native <code>Bun.serve</code> shell. Nothing is built on Bun, so the Node install step is unchanged (Drizzle has no client codegen step to run). Note: a direct <code>bun webjs.js start</code> bypasses npm lifecycle hooks, so any per-app asset a <code>prestart</code> hook generates (Tailwind css, generated component sources) must be baked at BUILD time in the image; only runtime-dependent steps (a DB <code>webjs db migrate</code>) belong in the start command. One trade-off: <code>Bun.serve</code> has no informational-response API, so the 103 Early Hints modulepreload head-start (covered above) is node-only. The preload hints still ship in the document head, so this costs a small first-load latency edge only where your edge forwards 103, not correctness.</li>

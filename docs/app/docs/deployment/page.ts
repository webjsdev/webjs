...
{ "version": "0.8.10", "build": "&lt;importmap-hash&gt;", "node": "v24.4.0", "uptime": 38.21 }</pre>
    <p><code>version</code> is the <code>@webjsdev/server</code> framework version, <code>build</code> is the published build id (the same value the client router reads from <code>data-webjs-build</code> to detect a deploy; empty until the vendor map resolves). It folds in a per-deploy fingerprint (<code>WEBJS_BUILD_ID</code> or a platform commit id) when one is set, so an SSR-only deploy changes it too, <code>node</code> is the running Node version, and <code>uptime</code> is process uptime in seconds. The response carries <code>Cache-Control: no-store</code>.</p>
...
Deno.serve({ port: 8080 }, (req) =&gt; app.handle(req));</pre>
    ...
    <ul>
      ...
      <li><strong>WEBJS_BUILD_ID</strong>: an optional per-deploy id (set it to your git SHA) folded into the client-facing build id so an <em>SSR-only</em> deploy (a template edit, a copy change, server-rendered syntax highlighting) is detected by the client router and reflected without a manual refresh. The importmap-hash build id alone does not change on an SSR-only deploy, so without this the client keeps serving pre-deploy HTML on soft navigation. Auto-detected from a platform commit id when present (<code>RAILWAY_GIT_COMMIT_SHA</code>, <code>RAILWAY_DEPLOYMENT_ID</code>, <code>VERCEL_GIT_COMMIT_SHA</code>, <code>RENDER_GIT_COMMIT</code>, <code>GIT_COMMIT</code>, <code>SOURCE_COMMIT</code>, <code>SOURCE_VERSION</code>), so on those platforms it works with no setup. All instances of one deploy must share the value.</li>
    </ul>

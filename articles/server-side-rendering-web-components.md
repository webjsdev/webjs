...
Because the content is in the first response, a WebJs page reads with JavaScript turned off, and interactivity is layered on per behaviour once the module loads. That is progressive enhancement, and server-rendered web components are what make it possible instead of aspirational. It also means search engines index the real content straight from that first response, whether the component rendered in light DOM or shadow DOM, with no JavaScript to execute and no shadow tree to reassemble.

...
/*
 * The WebJs 3 landing page, rebuilt on web components. The page is server
 ...
 * <section-nav> scroll spy). Content and copy are ported from the WebJs
 * site's landing-content module.
 */

const GH_URL = 'https://github.com/webjsdev/webjs';

// Homepage-only for now, so the routes not built here (blog, jam) point at
// their destination pages rather than 404ing on a local path.
const NAV_ITEMS = [
  { key: 'G', label: 'github', href: GH_URL },
  { key: 'D', label: 'docs', href: 'https://docs.webjs.dev' },
  { key: 'B', label: 'blog', href: 'https://webjs.dev/blog' },
  { key: 'J', label: 'jam', href: 'https://webjs.dev/jam/2026' },
  { key: 'S', label: 'store', href: 'https://shop.webjs.dev/' },
];

const CODE_SNIPPET = `import { type Handle, on } from 'webjs/ui'
import button from 'webjs/ui/button'
...
const SECTIONS: Section[] = [
  ...
  },
  {
    ...
    body: "WebJs ships with skills that help your AI agent learn the API and follow best practices. Whether you let the agent write all the code, or you tweak it by hand, WebJs just works. It's one unified stack that speaks WebJs end to end, not a patchwork of tools. When you want to change something, explain it in plain language. The framework stays out of your way.",
    panelClass: 'rmx-col-left-4',
  },
  {
    ...
    body: 'WebJs components build on web primitives like EventTarget and avoid the runtime semantics of React hooks, giving you back normal JavaScript control flow and execution. This works seamlessly with the web, including web components and third-party libraries. WebJs also provides native mixins for the DOM that make it easier than ever to compose and apply complex behavior on native platform elements.',
  ...
  },
  {
    ...
    body: 'Whatever you want to build, WebJs can meet the project where it is. Start something new, grow it into a business, or bring WebJs into an app that already exists. One technology, used in whatever way the project needs.',
    panelClass: 'rmx-col-right-full',
  },
  {
    id: 'start-building',
    kicker: 'Describe the destination',
    title: 'Building with WebJs can take you there',
    body: 'WebJs 3 is currently available as a beta release.',
    panelClass: 'rmx-col-left-6',
    cta: { label: 'Watch the repo', href: GH_URL },
    secondary: {
      kicker: 'Subscribe to our newsletter',
      title: 'Stay in the loop',
      body: 'Once a month, we write about everything in the world of WebJs. Sign up to be notified about progress on WebJs 3. No spam. Unsubscribe anytime.',
    },
  },
];
      ...
      <footer class="rmx-footer">
        <div class="rmx-footer-content">
          ...
          <div class="rmx-legal">
            ...
            <p>Built with WebJs. No build step, works without JavaScript.</p>
          </div>
        </div>
      </footer>

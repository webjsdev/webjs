...
export const PRESET_GLSL = /* glsl */ `
    ...
    out vec3 pos, out vec3 col)
  {
    ...
    if (idx < nebStart) {
      ...
      float lumBoost = 1.0;
      // Baryonic-gas temperature scale: recolour ~15% of the stars (the field
      ...
      if (tcol > 0.985)      { hue = 0.14;  sat = max(sat, 0.60); lumBoost = 1.6; } // ~1.5% hottest: white-gold
      else if (tcol > 0.940) { hue = 0.045; sat = max(sat, 0.85); lumBoost = 1.25; } // ~4.5% warm-hot: orange-red
      else if (tcol > 0.850) { hue = 0.71;  sat = max(sat, 0.65); }                // ~9% cold void: blue-violet

...
GET on the page (#240): the buffered HTML response gets a content-hash
`ETag` and a repeat request whose `If-None-Match` matches it returns a
`304 Not Modified` with no body. A `no-store` or `private` page gets NO
ETag and never 304s, so private / per-user content is never revalidated
across sessions. A streamed Suspense response is not ETagged. See the
conditional-GET section in the framework root `AGENTS.md`.

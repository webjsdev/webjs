...
suite('ui-select', () => {
  suiteSetup(async () => {
    await import(`${COMPONENTS_DIR}/select.ts`);
  });

  teardown(() => { cleanupPortals(); });

  test('trigger renders with aria-haspopup="listbox"', async () => {
    const root = await mount(html`
      <ui-select value="">
        <ui-select-trigger>
          <ui-select-value placeholder="Pick"></ui-select-value>
        </ui-select-trigger>
        <ui-select-content>
          <ui-select-item value="a">Apple</ui-select-item>
          <ui-select-item value="b">Banana</ui-select-item>
        </ui-select-content>
      </ui-select>
    `);
    await tick();
    const btn = root.querySelector('ui-select-trigger button');
    assert.ok(btn);
    assert.equal(btn.getAttribute('aria-haspopup'), 'listbox');
    root.remove();
  });

  test('opening renders content as a portal (role=listbox)', async () => {
    const root = await mount(html`
      <ui-select value="">
        <ui-select-trigger>
          <ui-select-value placeholder="Pick"></ui-select-value>
    ...
    `);
    await tick();
    const sel = root.querySelector('ui-select');
    sel.setOpen(true);
    await tick();
    const portal = document.body.querySelector('[data-slot="select-content"]');
    assert.ok(portal);
    assert.equal(portal.getAttribute('role'), 'listbox');
    root.remove();
  });

  test('clicking an item updates root value + closes', async () => {
    const root = await mount(html`
      <ui-select value="">
        <ui-select-trigger>
          <ui-select-value placeholder="Pick"></ui-select-value>
    ...
    await tick();
    const portal = document.body.querySelector('[data-slot="select-content"]');
    const items = portal.querySelectorAll('ui-select-item');
    items[1].click();
    await tick();
    assert.equal(sel.value, 'banana');
    assert.equal(sel.open, false, 'closes after pick');
    root.remove();
  });

  test('change event fires on item pick', async () => {
    const root = await mount(html`
      <ui-select value="">
        <ui-select-trigger>
          <ui-select-value placeholder="Pick"></ui-select-value>
    ...
    const sel = root.querySelector('ui-select');
    let detail = null;
    sel.addEventListener('change', (e) => { detail = e.detail; });
    sel.setOpen(true);
    await tick();
    const portal = document.body.querySelector('[data-slot="select-content"]');
    portal.querySelector('ui-select-item').click();
    await tick();
    assert.equal(detail?.value, 'apple');
    root.remove();
  });

  test('value attribute initializes selected item state', async () => {
    const root = await mount(html`
      <ui-select value="banana">
        <ui-select-trigger>
          <ui-select-value placeholder="Pick"></ui-select-value>
        </ui-select-trigger>
        <ui-select-content>
    ...
    `);
    await tick();
    const banana = root.querySelector('ui-select-item[value="banana"]');
    assert.equal(banana.getAttribute('data-state'), 'checked');
    root.remove();
  });
});

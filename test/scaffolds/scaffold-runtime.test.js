...
test('bun scaffold: agent-config markdown shows bun commands, no npm commands', async () => {
  ...
  try {
    await scaffoldApp('bunapp', cwd, { template: 'full-stack', runtime: 'bun' });
    const appDir = join(cwd, 'bunapp');
    for (const f of ['AGENTS.md', 'CONVENTIONS.md', '.cursorrules', '.agents/rules/workflow.md', '.github/copilot-instructions.md']) {
...
test('bun scaffold works across all three templates', async () => {
  for (const template of ['full-stack', 'api', 'saas']) {
    ...
    try {
      ...
      assert.match(read(appDir, 'Dockerfile'), /FROM oven\/bun:1/, `${template}: bun Dockerfile`);

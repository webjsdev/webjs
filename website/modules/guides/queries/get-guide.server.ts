...
import type { GuideWithBody } from '#modules/guides/types.ts';
...
/**
 * Read a single `guides/<slug>.md` and return its metadata + body.
 ...
 * includes the `slug` arg; the per-page `guide:` tag (plus the shared
 * `guides` tag) lets a future write path evict it.
 */
export const method = 'GET';
export const cache = { maxAge: 300, public: true };
export const tags = (slug: string) => ['guides', `guide:${slug}`];
export async function getGuide(slug: string): Promise<GuideWithBody | null> {
  if (!/^[a-z0-9-]+$/.test(slug)) return null;
  let raw: string;
  try { raw = await readFile(join(GUIDES_DIR, slug + '.md'), 'utf8'); }
  catch { return null; }

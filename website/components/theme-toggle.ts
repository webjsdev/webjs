...
export class ThemeToggle extends WebComponent {
  ...
  render() {
    ...
    return html`
      <button
        class="inline-flex items-center justify-center w-8.5 h-8.5 p-0 rounded-lg text-fg-muted cursor-pointer transition-all duration-150 hover:text-fg hover:bg-bg-subtle active:scale-[0.94] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/20"
        @click=${() => this.cycle()}

...
webjs db &lt;verb&gt;       # any verb the webjs.db block maps (bring your own ORM)</code-block>
    <p>The four kit verbs default to drizzle-kit, and a <code>webjs.db</code> block in <code>package.json</code> remaps any verb (or adds one) to your own ORM's shell command, so <code>webjs db migrate</code> keeps one spelling across ORMs. See <a href="/docs/database#bring-your-own-orm">Bring your own ORM</a>.</p>
...
&#125;</code-block>
    ...
    <h3>webjs ci</h3>
    <code-block>webjs ci                    # run the step list declared under "webjs": { "ci" } in package.json
webjs ci --only Tests       # one step or group by title (repeatable, case-insensitive)
webjs ci --fail-fast        # stop after the first failing step (-f)
webjs ci --json             # one JSON document on stdout, the human report on stderr
webjs ci --signoff          # after a green run, post a green commit status via gh signoff</code-block>
    <p>Local CI, the Rails <code>bin/ci</code> model: your machine is the first CI runner. Each step prints a heading, then <code>✅ &lt;title&gt; passed in 2.11s</code> or <code>❌ &lt;title&gt; failed in 0.01s</code>; the run ends with every failure listed and one total line, and exits 1 on any failure. A cloud pipeline runs the same list by calling <code>npm run ci</code>, so the two cannot drift, which is exactly what the scaffold's one-job GitHub workflow does (<code>webjs create --skip-ci</code> omits the workflow; the local list always ships).</p>
    <code-block>&#123;
  "webjs": &#123;
    "ci": &#123;
      "steps": [

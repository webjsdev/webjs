...
const FEATURES = [
  ...
  { icon: '🧱', title: 'Web Components · Light DOM by Default', desc: 'Lit-inspired component model on top of standard HTML custom elements — html`` / css`` tagged templates, static properties, ReactiveController protocol (Lit-compatible), Task and Context controllers. Light DOM is the default so Tailwind and global CSS apply directly; flip static shadow = true for scoped styles, <slot> projection, or embed isolation. Both modes SSR fully (light DOM as direct HTML, shadow DOM via DSD) and hydrate with zero flash. Editor intelligence in VS Code and Neovim via @webjskit/ts-plugin — type-checked templates, custom-element go-to-definition, attribute auto-complete from static properties — with one install and one tsconfig entry.' },

...
A PR must not merge until local CI is green on its pushed head (#1474). The gate is `npm run ci -- --signoff`: it runs the root `webjs.ci` list (everything the GitHub workflow runs, in `Gate` slots) and, only when every step passes, `gh signoff` posts a green `signoff` commit status on the pushed head, which `main`'s protection requires, plus one approving review from a CODEOWNER (`.github/CODEOWNERS` names @vivek7405 for every path, so that means the maintainer). Push first, then sign off; a later push has no signoff until the list is run again on it. Sign off from a REAL install, not a linked worktree (bare `@webjsdev/*` specifiers resolve into the primary there, so the run is partly vacuous). During the transition the six `ci.yml` job names stay required too; `scripts/protect-main.sh` declares the whole gate and `--local-only` is the final state. If `gh api repos/webjsdev/webjs/branches/main/protection` shows `required_status_checks: null`, run `bash scripts/protect-main.sh` once (needs repo admin) to restore it. Do not work around a red step. Fix whatever is red, re-run the list, and sign off again.

The gate is read ONCE, at merge, never in a mid-work sleep loop: `gh signoff status` (or `gh api repos/webjsdev/webjs/commits/<sha>/status`) for the signoff, and, while the Actions checks are still required, `gh pr checks` in full rather than trusting the merge button, because `ci.yml` defines roughly twice as many jobs as `main` requires.
